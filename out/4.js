"use strict";
function shortestPath(start, stations, target, max) {
    const distances = Array(max + 1).fill(Infinity);
    distances[start] = 0;
    let queue = [];
    ...
    while (queue.length > 0) {
        ...
        if (currentNode !== -1) {
            ...
            const otherStations = stations.filter((station) => station !== currentNode);
...
function permute(shops, index, length, p) {
    if (index === length) {
...
function minEnergy(start, shops, stations, target) {
    ...
    for (const shopsComb of permutedShops) {
        ...
        distance = shortestPath(shopsComb[shopsComb.length - 1], stations, target, max);
        totalDistance += distance;
